...
exports.listAssets = async (req, res) => {
  try {
    ...
    res.render('assets/list', { assets, employees, assignments });
  } catch (err) {
...
exports.createAsset = async (req, res) => {
  try {
    ...
    if (!name || isNaN(price) || isNaN(quantity) || price <= 0 || quantity <= 0) {
      return res.status(400).send('Invalid asset data');
    }
...
exports.editRentedAssignment = async (req, res) => {
  try {
    ...
    if (!assetId || !employeeId || !month || isNaN(quantity) || quantity < 0) {
      return res.status(400).send('Invalid rented assignment data');
    }
